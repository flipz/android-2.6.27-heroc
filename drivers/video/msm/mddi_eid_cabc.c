		       ...
		       enum led_brightness brightness)
{
	...
	if ((test_bit(LS_STATE, &cabc->status)) ||
	    (test_bit(SUSPEND, &cabc->status))  ||
...
retry:
	...
	if (on) {
		...
		/* led core will issue brightness_set() on this */
...
ls_set_brightness(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	struct cabc_config *cabc_config = cabc->cabc_config;
	struct ls_alg_classdev *alg_cdev = &cabc->alg_cdev;
	int brightness;
	...
	brightness = cabc_config->pwm_data[level];
...
cabc_bl_handle(struct platform_device *pdev, int brightness)
{
	...
	struct led_classdev *lcd_cdev, *ls_cdev;
	int screen_bright = DEFAULT_BRIGHTNESS;
	...
	ls_cdev = &cabc->light_sensor_dev;
	wake_lock(&cabc->wakelock);

	if (brightness != LED_FULL) {
		clear_bit(GATE_ON, &cabc->status);
	} else {
		if (test_bit(LS_SWITCH, &cabc->status)) {
			/* uP already fires */
			if (ls_cdev->brightness)
				screen_bright = ls_cdev->brightness;
			__turn_on_backlight(cabc, screen_bright);
		} else
	      ...
	      const char *buf, size_t count)
{
	...
	case LIGHT_SENSOR:
		if (ls_on(cabc, !!res) >= 0)
			change_bit(LS_STATE, &cabc->status);
		else
			count = -EIO;
		break;
	case AUTO_BACKLIGHT:
...
samsung_cabc_suspend(struct early_suspend *h)
{
	...
	struct led_classdev *ls_cdev = &cabc->light_sensor_dev;
	int res;
	...
	if (res != NOTIFY_STOP) {
		...
		/* clear light sensor. On resume, we can know
		 * if uP triggers or not */
		ls_cdev->brightness = 0;

		if (test_bit(LS_STATE, &cabc->status)) {
			led_classdev_suspend(ls_cdev);
			cancel_work_sync(&cabc->ls_changed_work);
		}
...
static void setup_alg(struct cabc *cabc)
{
	struct cabc_config *config = cabc->cabc_config;
	int level = config->min_level;

	cabc->alg_cdev.name = "cabc_ls_alg";
	cabc->alg_cdev.min_level = level;
	cabc->alg_cdev.min_brightness = config->pwm_data[level];
