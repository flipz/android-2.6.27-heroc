		...
		struct msm_mddi_client_data *client_data)
{
	...
	if (bridge->panel_type == PANEL_EID_40pin)
		...
		(bridge->panel_type == PANEL_HEROC_TPO))
	...
	if ((bridge->panel_type == PANEL_HEROC_EID_BOTTOM) ||
	...
	if ((bridge->panel_type == PANEL_HEROC_TPO) ||
	    (bridge->panel_type == PANEL_HEROC_EID_BOTTOM))
...
static int mddi_samsung_probe(struct platform_device *pdev)
{
	...
	struct msm_mddi_bridge_platform_data *bridge_data =
	    client_data->private_client_data;
	cabc_config.pwm_data = bridge_data->pwm;
	cabc_config.min_level = bridge_data->min_level;
	B(KERN_DEBUG "%s: enter\n", __func__);
	...
	if ((bridge_data->panel_type == PANEL_EID_40pin) ||
		...
		(bridge_data->panel_type == PANEL_HEROC_EID_BOTTOM) ||
		(bridge_data->panel_type == PANEL_HEROC_TPO)) {
