...
static void alg_update_work(struct work_struct *work)
{
	...
	printk(KERN_INFO "%s: Desire(%d %d), Current(%d %d)\n",
			__func__, alg_cdev->desire_level + 1,
			alg_cdev->desire_brightness, alg_cdev->level + 1,
			alg_cdev->brightness);
	...
	if (tmp <= alg_cdev->min_brightness) {
		alg_cdev->brightness = alg_cdev->min_brightness;
		alg_cdev->level = alg_cdev->min_level;
		goto end;
	}
