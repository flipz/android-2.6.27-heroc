...
static struct _melfas_ts_version melfas_ts_version;
...
static int melfas_ts_report(struct melfas_ts_data *ts)
{
	...
	static uint8_t touch_panel_is_pressed = 0;
	static uint16_t position_data[2][2];

	int buf_len = (melfas_ts_version.sensor_revision == MELFAS_DIAMOND_PATTERN)? 11 : 10;
...
/*	if (melfas_ts_version.sensor_revision == MELFAS_DIAMOND_PATTERN) {
		if (buffer[9])
			printk("2nd button = %d\n", (buffer[6] & 0x80) << 1|buffer[9]);
	}*/
	if (buffer[buf_len - 1])
	...
	if (key_x_axis) {
		for (loop_i = 0; loop_i < ts->virtual_key_num; loop_i++) {
			if (key_x_axis >= ts->virtual_key_value[loop_i].range_min &&
				key_x_axis <= ts->virtual_key_value[loop_i].range_max &&
				!ts->virtual_key_value[loop_i].status) {
				ts->virtual_key_value[loop_i].status = 1;
				input_event(ts->input_key_dev, ts->virt_key_type,
			...
			if ((key_x_axis < ts->virtual_key_value[loop_i].range_min ||
				key_x_axis > ts->virtual_key_value[loop_i].range_max) &&
				ts->virtual_key_value[loop_i].status) {
				ts->virtual_key_value[loop_i].status = 0;
				input_event(ts->input_key_dev, ts->virt_key_type,
	...
	if(!(buffer[0] & 0x3)){
		for (loop_i = 0; loop_i < 4; loop_i++) {
			if(ts->virtual_key_value[loop_i].status) {
				ts->virtual_key_value[loop_i].status = 0;
				input_event(ts->input_key_dev, ts->virt_key_type,
					ts->virtual_key_value[loop_i].keycode, 0);
			}
	...
	/* handle draw screen */
	if(buffer[0] & 0x3) {
		touch_panel_is_pressed = 1;
		finger_num = buffer[0] & 0x3;
		if(press_pressure) {
			/* single finger */
			position_data[0][0] =  buffer[1] | (buffer[0] & 0x18) << 5;
			position_data[0][1] =  buffer[2] | (buffer[0] & 0x60) << 3;
			input_report_abs(ts->input_dev, ABS_X, position_data[0][0]);
			input_report_abs(ts->input_dev, ABS_Y, position_data[0][1]);
		}
		input_report_abs(ts->input_dev, ABS_PRESSURE, press_pressure);
		input_report_abs(ts->input_dev, ABS_TOOL_WIDTH, press_width);
		...
		if (finger2_pressed) {
			...
			input_report_abs(ts->input_dev, ABS_HAT0X, position_data[1][0]);
			input_report_abs(ts->input_dev,	ABS_HAT0Y, position_data[1][1]);
		}
		input_sync(ts->input_dev);
	} else
	if(touch_panel_is_pressed){
		input_report_key(ts->input_dev, BTN_TOUCH, 0);
		input_report_key(ts->input_dev, BTN_2, 0);
		input_sync(ts->input_dev);
		touch_panel_is_pressed = 0;
	}
...
{
	int ret = 0;
	sprintf(buf, "%s_%2.2X_%2.2X_%2.2X_%2.2X\n",
		MELFAS_I2C_RMI_NAME,
		melfas_ts_version.sensor_revision,
		melfas_ts_version.hw_revision,
		melfas_ts_version.comp_group,
		melfas_ts_version.fw_version);
	ret = strlen(buf) + 1;
	return ret;
}
...
#ifdef ENABLE_MELFAS_TOUCH_DEBUGFS
static int16_t debugfs_addr = 0;
static int16_t debugfs_data_length = 0;
	...
	char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if(debugfs_addr && debugfs_data_length) {
		...
		for(loop_i = 0; loop_i < debugfs_data_length; loop_i++)
			ret += scnprintf(buf + ret, PAGE_SIZE - ret,"0x%2.2X ",
			data_tmp[loop_i]);
		ret += scnprintf(buf + ret, PAGE_SIZE - ret, "\n");
	} else
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	memset(buffer, 0x0, sizeof(buffer));
	ret = melfas_i2c_read_block(client,
	...
	printk(KERN_INFO "%s: detected done, OP Mode: 0x%x\n",
		__func__, buffer[0]);
	...
	melfas_ts_version.sensor_revision = buffer[0];
	...
	melfas_ts_version.hw_revision = buffer[0];
	...
	melfas_ts_version.comp_group = buffer[0];

	while (pdata->version > melfas_ts_version.sensor_revision)
			pdata++;
	...
	melfas_ts_version.fw_version = buffer[0];
	...
	max_y = (buffer[0] & 0x3) << 8 | buffer[1];

	printk(KERN_INFO "%s: sensor_rev: 0x%2.2X, hw_rev: 0x%2.2X, comp_group: 0x%2.2X\n"
		"\tfw_ver: 0x%X, max X size: %d, max Y size: %d\n",
		__func__, melfas_ts_version.sensor_revision,
		melfas_ts_version.hw_revision,
		melfas_ts_version.comp_group,
		melfas_ts_version.fw_version,
		max_x, max_y);
