...
static unsigned int heroc_row_gpio_pow_save[] = { 40 };
...
static int heroc_row_gpios_power_saving(
		const struct gpio_event_platform_data *pdata, bool gpio_switch)
{
	uint8_t loop_i;

	if (!gpio_switch) {
		for (loop_i = 0; loop_i <
			ARRAY_SIZE(heroc_row_gpio_pow_save); loop_i++) {
			disable_irq(
				gpio_to_irq(heroc_row_gpio_pow_save[loop_i]));
		}
	} else {
		for (loop_i = 0; loop_i <
			ARRAY_SIZE(heroc_row_gpio_pow_save); loop_i++) {
