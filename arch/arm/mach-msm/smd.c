...
#if (CONFIG_MSM_SMD_DATA5_64K)
#define SMD_LARGE_BUF_SIZE 65536
#define SMD_NORMAL_BUF_SIZE 8192
#else
#define SMD_LARGE_BUF_SIZE 8192
#define SMD_NORMAL_BUF_SIZE 8192
#endif
...
#if (CONFIG_MSM_SMD_DATA5_64K)
struct smd_normal_fifo_data
{
	unsigned char ch0_normal_data[SMD_NORMAL_BUF_SIZE];
	unsigned char ch1_normal_data[SMD_NORMAL_BUF_SIZE];
};

struct smd_large_fifo_data
{
	unsigned char ch0_large_data[SMD_LARGE_BUF_SIZE];
	unsigned char ch1_large_data[SMD_LARGE_BUF_SIZE];
};
#endif
...
struct smd_channel {
	...
	volatile unsigned char *tx_share_fifo;
	volatile unsigned char *rx_share_fifo;
...
static void smd_channel_probe_worker(struct work_struct *work)
{
	...
	for (n = 0; n < 64; n++) {
		...
		if (!shared[n].name[0])
			continue;
		if(!smd_alloc_channel(shared[n].name,
				  shared[n].cid,
				  shared[n].ctype))
...
{
	return (ch->recv->head - ch->recv->tail) & (ch->fifo_sz - 1);
}
...
{
	return (ch->fifo_sz - 1) -
		((ch->send->head - ch->send->tail) & (ch->fifo_sz - 1));
}
...
static unsigned ch_read_buffer(struct smd_channel *ch, void **ptr)
{
	...
	*ptr = (void*)(ch->rx_share_fifo + tail);

	if (tail <= head)
		return head - tail;
	else
		return ch->fifo_sz - tail;
}
...
static void ch_read_done(struct smd_channel *ch, unsigned count)
{
	...
	ch->recv->tail = (ch->recv->tail + count) & (ch->fifo_sz - 1);
	ch->send->fTAIL = 1;
}
...
static unsigned ch_write_buffer(struct smd_channel *ch, void **ptr)
{
	...
	*ptr = (void *) (ch->tx_share_fifo + head);

	if (head < tail) {
		return tail - head - 1;
	} else {
		if (tail == 0)
			return ch->fifo_sz - head - 1;
		else
			return ch->fifo_sz - head;
	}
}
...
static void ch_write_done(struct smd_channel *ch, unsigned count)
{
	...
	ch->send->head = (ch->send->head + count) & (ch->fifo_sz - 1);
	ch->send->fHEAD = 1;
}
...
#ifdef CONFIG_MSM_RMNET_STREAM_MODE
	if (chn == 11)
		return 0;
#endif
...
static int smd_alloc_channel(const char *name, uint32_t cid, uint32_t type)
{
	...
	struct smd_large_fifo_data *large_fifo_data;
	struct smd_normal_fifo_data *normal_fifo_data;
	...
	if (cid == 11) { /*DATA5*/
#if (CONFIG_MSM_SMD_DATA5_64K)
		ch->fifo_sz = SMD_LARGE_BUF_SIZE;
		large_fifo_data = smem_alloc(SMEM_FIFO_BASE_ID + cid, sizeof(*large_fifo_data));

		if(large_fifo_data)
		{
			ch->tx_share_fifo = large_fifo_data->ch0_large_data;
			ch->rx_share_fifo = large_fifo_data->ch1_large_data;

		}
		else
			printk(KERN_INFO "smem_alloc: large_fifo_data fail");

#else
		ch->fifo_sz = SMD_LARGE_BUF_SIZE;
		ch->tx_share_fifo = &shared->ch0.data[0];
		ch->rx_share_fifo = &shared->ch1.data[0];
	...
	{
		ch->fifo_sz = SMD_NORMAL_BUF_SIZE;
#if (CONFIG_MSM_SMD_DATA5_64K)
		normal_fifo_data = smem_alloc(SMEM_FIFO_BASE_ID + cid, sizeof(*normal_fifo_data));

		ch->tx_share_fifo = normal_fifo_data->ch0_normal_data;
		ch->rx_share_fifo = normal_fifo_data->ch1_normal_data;
#else
		ch->tx_share_fifo = &shared->ch0.data[0];
		ch->rx_share_fifo = &shared->ch1.data[0];
#endif			
	...
	pr_info("smd_alloc_channel() '%s' cid=%d, shared=%p, tx_share_fifo=%p, rx_share_fifo=%p \n",
		ch->name, ch->n, shared, ch->tx_share_fifo, ch->rx_share_fifo);
