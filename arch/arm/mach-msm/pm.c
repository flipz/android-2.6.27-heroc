...
static int msm_pm_modem_busy_count_max = CONFIG_MSM7XXX_MODEM_BUSY_COUNT_MAX;
module_param_named(modem_busy_count_max, msm_pm_modem_busy_count_max, int, S_IRUGO | S_IWUSR | S_IWGRP);
static int modem_busy_count;
...
enter_failed:
	if (enter_state) {
		...
		/* ARM9 early exit */
		if (smsm_get_state() & SMSM_PWRC_EARLY_EXIT)
			rv = -EBUSY;
...
#endif
		(modem_busy_count < msm_pm_modem_busy_count_max) &&
...
#if 0
		if (ret == -EBUSY)
			pr_info("=========== %s: early exit\n", __func__);
		pr_info("=========== %s: modem_busy_count %d\n", __func__, modem_busy_count);
#endif
#ifdef CONFIG_MSM_IDLE_STATS
		if (ret)
