...
module_param_named(debug_mask, smd_rpcrouter_debug_mask, int,
...
static void do_read_data(struct work_struct *work)
{
	...
	if (smd_rpcrouter_debug_mask & SMEM_LOG) {
		...
		if (rq->xid == 0)
			smem_log_event6(SMEM_LOG_PROC_ID_APPS
					| RPC_ROUTER_LOG_EVENT_MID_READ,
					PACMARK_MID(pm),
					hdr.dst_cid, hdr.src_cid,
					ntohl(rq->prog),
					ntohl(rq->vers),
					ntohl(rq->procedure));
		else
	...
	)
{
	...
	if (r_ept->tx_quota_cntr == RPCROUTER_DEFAULT_RX_QUOTA) {
		...
		if (smd_rpcrouter_debug_mask & SMEM_LOG) {
			if (rq->xid == 0)
				...
				hdr->dst_pid, hdr->dst_cid, hdr->src_cid,
				ntohl(rq->prog),
				ntohl(rq->vers),
				ntohl(rq->procedure));
			else
	...
	spin_unlock_irqrestore(&smd_lock, flags);
       ...
       if (smd_rpcrouter_debug_mask & SMEM_LOG) {
		...
		PACMARK_MID(pacmark), hdr->dst_cid, hdr->src_cid,
		ntohl(rq->prog),
		ntohl(rq->vers),
		ntohl(rq->procedure));
       else
		...
		ntohl(rq->xid), hdr->dst_cid, hdr->src_cid,
		ntohl(rq->prog),
		ntohl(rq->vers),
		ntohl(rq->procedure));
	}
